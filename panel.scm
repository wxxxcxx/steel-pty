;; Bottom-panel terminal implementation for Helix configurations that need
;; workspace controls and side-panel insets. Keep this separate from term.scm
;; so the upstream implementation can be updated without resolving this fork's
;; UI changes inside the upstream file.
...
(define (push-terminal-component! term)
...
(define (show-term term)
  ...
  ;; An active terminal can sit below a component opened later, such as the
  ;; file tree. Remove that instance by name before pushing it back on top.
...
(define *terminal-inset-left* 0)
(define *terminal-inset-right* 0)

(define (set-terminal-horizontal-insets! left right)
  (set! *terminal-inset-left* (max 0 left))
  (set! *terminal-inset-right* (max 0 right))
  (set! stashed-area #f))
  ...
  (if (and terminal-area (equal? stashed-area rect))
      ...
      (begin
        ...
        (define available-width
          (max *min-term-width*
               (- (area-width rect) *terminal-inset-left* *terminal-inset-right*)))
        (define panel-height
          (if *terminal-fullscreen?*
              available-height
              (max 6 (round (* *terminal-fraction* available-height)))))
        (set! *default-terminal-rows* panel-height)
        (set! *default-terminal-cols* available-width)
        ...
        (set! terminal-area
              (area (+ (area-x rect) *terminal-inset-left*)
...
(define (raise-terminal-if-active!)
